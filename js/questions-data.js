...
try {
    validAssessment = Object.freeze({
        ...
        questions: [
            ...
            {
                id: "AC-01",
                text: "Please select '4 - Moderately characteristic' for this statement: I am paying attention to this survey.",
            ...
            {
                id: "AC-02",
                text: "For quality control purposes, please select 'Not at all characteristic' (1) for this item.",
                dimension: "attention_check",
                category: "quality",
                correctAnswer: 1
            }
    ...
    // Validate the frozen object
    if (!validAssessment.questions || !Array.isArray(validAssessment.questions)) {
        throw new Error('Invalid questions array structure');
    }

    const attentionChecks = validAssessment.questions.filter(q => q.dimension === 'attention_check');
    if (attentionChecks.length !== 2) {
        throw new Error(`Expected 2 attention check questions, found ${attentionChecks.length}`);
    }

    console.log('Questions data module initialized successfully:', {
        totalQuestions: validAssessment.questions.length,
        attentionChecks: attentionChecks.length
    });

} catch (error) {
    console.error('Error initializing questions data:', error);
    throw error;
}

// Export the frozen assessment data
